...
{
    internal class CompraSubscribe : ICompraSubscribe
    {
        ...
        {
            string EventName = $"COMPRA.{typeof(Order).Name}";

            var queue = Channel.QueueDeclare("COMPRA", false, false, false, null);
            ...
            consumer.Received += async (sender, evenArgs) =>
            {
                ...
                using (var scope = ServiceScopeFactory.CreateScope())
                {
                    var handler = scope.ServiceProvider
                                       .GetService<INotificarCompraService>();

                    await handler.Handle(EventObject);
                }
            };
            Channel.BasicConsume("COMPRA", true, consumer);
        }
    }
}
